...
export function onQueueITRequest(req: Request, conf: IntegrationDetails | null = null): Response | null {
    if (conf == null) {
    ...
    const integrationProvider = conf.provider == null ? new QueueItIntegrationEndpointProvider() : conf.provider;
    ...
    const queueItToken = Utils.getParameterByName(requestUrl, KnownUser.QueueITTokenKey);
    ...
    const validationResultPair = KnownUser.validateRequestByIntegrationConfig(requestUrlWithoutToken, queueItToken, integrationConfigJson,
        conf.customerId, conf.secretKey, httpProvider!);
    ...
    if (validationResultPair.first != null &&
        ...
        if (queueItToken != '' && !validationResult.isAjaxResult && validationResult.actionType == 'Queue') {
            let response = new Response(null, {
                status: 302,
                headers: httpProvider!.getHttpResponse().getHeaders(),
...
export function onQueueITResponse(res: Response): void {
    ...
    for (let i = 0; i < contextHeaderKeys.length; i++) {
        ...
        if (value != null && value!.length > 0) res.headers.set(contextHeaderKeys[i], value!);
    }
}
